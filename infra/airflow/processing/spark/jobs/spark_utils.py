...
def ensure_iceberg_table(
    ...
    *,
    ...
    LOG.info("Table ensured: %s", table)
    ...
    try:
        ...
        if fs.exists(path):
            logger.warning(
                ...
                "Spark will resume from the checkpoint. Use a new checkpoint path to replay from earliest offsets.",
                checkpoint,
            )
    except Exception as exc:  # pragma: no cover - defensive
        logger.info("Unable to verify checkpoint existence (%s); continuing.", exc)
