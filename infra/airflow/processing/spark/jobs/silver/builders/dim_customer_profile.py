...
def build_dim_customer_profile(spark: SparkSession, _: DataFrame | None) -> DataFrame:
    ...
    users = users_raw.select(
        F.col("payload.after.user_id").alias("user_id"),
        F.col("payload.after.email").alias("email"),
        F.col("payload.after.country").alias("country"),
        F.col("payload.after.created_at").alias("created_at_raw"),
    ...
    segments = segments_raw.select(
        F.col("payload.after.user_id").alias("user_id"),
        F.col("payload.after.segment").alias("segment"),
        F.col("payload.after.lifetime_value").alias("lifetime_value_raw"),
        unix_ms_to_ts(F.col("payload.ts_ms")).alias("change_ts"),
        "event_time",
        F.col("partition").alias("bronze_partition"),
        F.col("offset").alias("bronze_offset"),
    ...
    filled = (
        combined.withColumn("email", F.last("email", ignorenulls=True).over(fill_window))
    ...
    return dimensional.withColumn(
        "customer_sk", surrogate_key(F.col("user_id"), F.col("valid_from"))
    ).select(
        "customer_sk",
